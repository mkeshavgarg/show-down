...
class showDown:
    ...
    def downloadLatest(self,show_name):
        ...
        if temp != -1 :
            ...
            print ("Latest available episode: %s"%(self.filename))
            ...
            if response=='y' or response=='Y':
               self.downloader(url = self.url , filename = self.filename )
                ...
                print("Oh!! you did not download any episode")
                print("\rBye")
                return
            temp = -1
        else :
    ...
    def currentSeason(self,show_name):
        res = urllib2.urlopen(urllib2.Request(self.url, headers = self.header)).read()
        soup=bs4.BeautifulSoup(res, "html.parser")
        elems=soup.select('.data a')
    ...
    def latestEpisode(self,show_name):
        res = urllib2.urlopen(urllib2.Request(self.url, headers = self.header)).read()
        soup=bs4.BeautifulSoup(res, "html.parser")
        elems=soup.select('.data a')
